...
export function createJexlEditor(
...
// Call this in your React Monaco Editor's onMount prop
export function handleEditorDidMount(
  editor: monaco.editor.IStandaloneCodeEditor
) {
  ensureFontsLoaded(() => {
    monaco.editor.remeasureFonts();
    // Force a layout in case font metrics changed
    editor.layout();
  });
}
...
export function getJsonPathFromOffset(
  jsonString: string,
  offset: number
): string | null {
  try {
    ...
    return location.path.reduce<string>((prevPath, pathSeg, index) => {
      ...
      else if (/[`~!@#%^&*()|+\\\-=?;:'.,\s']/g.test(pathSeg))
        return `${prevPath}['${pathSeg}']`;
